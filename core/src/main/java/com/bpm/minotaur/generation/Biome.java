...
public enum Biome {
    ...
    FOREST(true, true, 30, applyTorchLighting(Color.LIGHT_GRAY, 5.0f, new Color()), "images/skybox/skybox_forest.png",
     ...
     * Creates a torch effect with full brightness close to player, fading to
    ...
    private static float calculateTorchBrightness(float distance) {
        if (distance <= TORCH_FULL_BRIGHTNESS_RADIUS) {
            ...
            float fadeRatio = (distance - TORCH_FULL_BRIGHTNESS_RADIUS)
                    / (TORCH_FADE_START - TORCH_FULL_BRIGHTNESS_RADIUS);
