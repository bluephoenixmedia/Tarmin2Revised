---
description: Wire dependency-cruiser into a TypeScript repo so each package is a deep module, with implementation hidden in subfolders and reachable only throug...
---
...
src/packages/
  <name>/
    index.ts        â† an entry point (public). Import this from outside.
    client.ts       â† another entry point. Packages may expose SEVERAL.
    lib/            â† implementation: hidden from outside, free to import each other.
    tests/          â† co-located tests + fixtures (a subfolder, so private).
```
...
- **Package manager**: `pnpm-lock.yaml` â†’ pnpm, `yarn.lock` â†’ yarn, `bun.lockb` â†’ bun, else npm. Use it for every command below (`pnpm`/`yarn`/`npm run`/`bunx`).
