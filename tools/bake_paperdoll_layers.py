...
def write_triage(path, unresolved, calibration):
    """The art worklist: what has no layer, and what has been flagged for re-creation.

    Written for whoever is running the generation session, so it carries what they need
    at the moment they need it -- the target size for the slot and the prompt to use --
    rather than just naming files and leaving them to go and look those up.
    """
    by_slot = {}
    ...
    # Per-slot target footprints, so the list is actionable without cross-referencing.
    targets = {
        "head": "330 x 426", "chest": "471 x 609", "arms": "395 x 277 per arm",
        "hands": "382 x 218 per hand", "legs": "459 x 665", "feet": "301 x 349 per boot",
        "shield": "411 x 450", "cloak": "719 x 876", "weapon": "386 x 772",
    }
    ...
    lines = [
        ...
        "",
        "## Flagged for re-creation (%d)" % len(redo),
        "",
    ...
    if not redo:
        lines += ["_Nothing flagged yet._", ""]
    else:
        lines += ["| Layer | Slot | Target size | Replace file |", "|---|---|---|---|"]
        for layer_id in redo:
            slot = layer_id.split("/", 1)[0]
            lines.append("| `%s` | %s | %s | `assets/images/paperdoll/%s.png` |"
                         % (layer_id, slot, targets.get(slot, "-"), layer_id))
        lines += [
            "",
...
def main():
    ...
    ap.add_argument("--triage", action="store_true",
                    help="only regenerate docs/paperdoll_triage.md from the calibration file")
    args = ap.parse_args()
    ...
    if args.triage:
        # Deliberately cheap: rebuilding the worklist should not mean re-hashing and
        # re-normalising 371 images. Flag pieces in the editor, run this, take the list
        # to a generation session.
        cal = load_existing_calibration(os.path.join(data_dir, "paperdoll_calibration.json"))

        # Read the mapping that already exists rather than rebuilding it, which would
        # mean re-hashing every image just to find out which items are unmapped.
        map_path = os.path.join(data_dir, "paperdoll_layers.json")
        mapped = set()
        if os.path.exists(map_path):
            with open(map_path, "r", encoding="utf-8") as fh:
                mapped = set(json.load(fh).get("items", {}))

        unresolved = []
        for name in ("armor.json", "weapons.json"):
            with open(os.path.join(data_dir, name), "r", encoding="utf-8") as fh:
                for key, item in json.load(fh).items():
                    if key in mapped:
                        continue
                    slot = slot_for_item(item) if name == "armor.json" else "weapon"
                    if slot:
                        unresolved.append((key, slot, [basename_no_ext(item.get("texturePath")) or key.lower()]))

        write_triage(os.path.join(repo, "docs", "paperdoll_triage.md"), unresolved, cal)
        flagged = sum(1 for c in cal.values() if c.get("needsArtRedo"))
        print("wrote docs/paperdoll_triage.md -- %d layers flagged for re-creation" % flagged)
        return
